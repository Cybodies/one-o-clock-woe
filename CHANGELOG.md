...
## [2026.08.30.1]
### Changed (เวลาขอประมูล → ช่วง 19:30–22:00 ทุกหน้า ทุกวันกิจ)
- **ขอประมูลได้เฉพาะ 19:30–22:00 ของวันกิจกรรม** (เดิม: เปิด 12:00 แล้วไม่มีเวลาปิด
  ตั้งแต่ v2026.08.01.3) — ใช้ช่วงเดียวกันทั้งหน้า **Auction GL** และ **Auction Overrun**
  และทั้ง อังคาร/พฤหัส (GL) · อาทิตย์ (Overrun). ปิด **22:00 แบบ exclusive** คือ 21:59 ยังขอได้
  22:00 ตรงคือปิดแล้ว.
- เกตยังอยู่ที่ `arRequestBlockReason` ที่เดียว (ใช้ทั้งตอนเปิด modal และตอนกดส่ง) —
  **ปิดเฉพาะการ *ขอ*ประมูล** เท่านั้น แอดมินอนุมัติ/จัดคิวหลัง 22:00 ได้ตามเดิม.
- **นาฬิกาเปลี่ยนเป็น "นาทีของวัน"**: `bkkHour()` (ชั่วโมงล้วน) เขียนไม่ได้ว่า 19:30 →
  แทนด้วย `bkkMinuteOfDay()` + `auctionWindowState()` ที่คืน `"early" | "late" | null`
  เป็นแหล่งความจริงเดียวของทั้งเกตและข้อความบนจอ.
- ตัวเลขมาจากค่าคงที่ 4 ตัว (`AUCTION_OPEN_HOUR/MINUTE`, `AUCTION_CLOSE_HOUR/MINUTE`)
  ป้ายทุกอันสร้างจาก `AUCTION_OPEN_LABEL` / `AUCTION_CLOSE_LABEL` — **ไม่มีเวลาฮาร์ดโค้ด
  ในข้อความ** (รอบก่อนหลุดคำว่า "ตอนเที่ยง" ค้างไว้ รอบนี้มีเทสไล่จับ).
- แท็บที่เปิดค้างไว้ re-render เองทั้ง **สองขอบ**: ปลดล็อกตอน 19:30 และปิดตอน 22:00
  (`checkAuctionWindowFlip` แทน `checkAuctionOpenFlip` — ขอบเดียวเดิม).
- เทส: harness เปลี่ยน `setHour(h)` → `setClock(h, m)` (คุม `bkkMinuteOfDay`) —
  ถ้าไม่เปลี่ยน เทสจะไปอ่านนาฬิกาจริงแล้วผ่านเฉพาะหลัง 19:30. เพิ่มเทสขอบ 19:29/19:30
  และ 21:59/22:00. **226/226 ผ่าน**.
